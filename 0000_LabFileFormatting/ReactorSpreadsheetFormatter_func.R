...
for (k in 20) { # use this line to pick one reactor
 # for (k in 1:length(reactor.file.names)) { # use this line to run format all reactors
  ...
  rm(list = ls()[!ls() %in% c("reactor.file.names0", "k", "path.data.red")])
  #jrk added the 0 at the end here
  
  # Get the current reactor data
  curr.reactordata <- reactor.file.names0[k]
