...
# V 08.2 adjusts reactor Twenty excel file name and hard codes the file directory (auto finder not working)
...
path.data.red <-  "/Users/juliakelson/Documents/TripleO/D17O_Data_Reduction_active"
...
formatCount = 0
for (reactor.file.number in 1: length(reactor.file.names) ) { # can be used for either UM or JHU files

   # for (reactor.file.number in 20) { # can use this for debugging
  ...
  if (formatCount == 0){
    ...
    source(paste(path.to.format.func, "ReactorSpreadsheetFormatter_func.R", sep = ""))
    
    # Run formatting function (no graphics, just formats lab excel files into a common CSV format)
      ReactorSpreadsheetFormatter_func(reactor.file.names)
  ...
  g1 <-  ggplot(curSMOW, aes(x=IPL.num, y=d33, group=1)) +
    geom_point(aes(shape=CorType),size=3) +
  ...
  grid::grid.draw(gB_d33_d34_SMOW) #JRK
  #dev.off() #JRK
  dev.off()
  if (!is.null(dev.list()))
    dev.off()
  ...
  pdf(file = paste(path.out, "basic_figures", "/", "R", reactor.file.number, "_", "2allCorr_SMOW.pdf",sep=""),   # The directory you want to save the file in
      width = 26, # The width of the plot in inches
      height = 18) # The height of the plot in inches
  grid::grid.draw(gb_allCorr_SMOW) #JRK
  dev.off() #JRK
  if (!is.null(dev.list()))
    dev.off()
  ...
  pdf(file = paste(path.out, "basic_figures", "/", "R", reactor.file.number, "_", "3allCorr_std.pdf",sep=""),   # The directory you want to save the file in
      width = 26, # The width of the plot in inches
      height = 18) # The height of the plot in inches
  grid::grid.draw(gB_allCorr_std) #JRK
  dev.off() #JRK
  if (!is.null(dev.list()))
    dev.off()
  ############################################################################################
  ...
  for (r in 1:length(names.Type.1)){
    ...
    grid::grid.newpage() #JRK
    grid::grid.draw(gB) #JRK
