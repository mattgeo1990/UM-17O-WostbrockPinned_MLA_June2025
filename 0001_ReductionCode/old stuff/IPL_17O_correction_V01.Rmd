...
# for(reactor.file.number in 1:length(reactor.file.names)) {
  ...
  ############################################################################################   # Search the master list for samples relevant to this reactor. Note that this is currently only applied to Reactors 11 and 13. 4/23/2020
  ...
  for (m in 1:length(data.cor)){
    curCarbStd <- subset(data.cor[[m]],data.cor[[m]]$Type.1 == "CarbonateStd" | data.cor[[m]]$Type.1 == "Carbonate")
    curCarbStd <- unique(curCarbStd$Type.2) # Only keep the relevant column
    if (length(curCarbStd)>1){ # there are >1 carbonate standards
      flag.mult.carbstd[m,1] <- length(curCarbStd) # number of different carbonate Type.2's analyzed. Number is just for user - anything above -9999 will run
    }
  }
