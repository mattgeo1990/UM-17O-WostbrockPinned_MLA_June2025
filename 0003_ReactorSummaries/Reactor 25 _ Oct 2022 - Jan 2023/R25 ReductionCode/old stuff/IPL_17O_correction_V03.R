...
# 2 - Always use 102-GC-AZ01 (keeps standard normalization most constant between reactors)
tert.cor.option <- 2
...
# for (reactor.file.number in 16) { # can be used for either UM or JHU files
  ...
  for (m in 1:length(data.cor)) {
    if (flag.mult.carbstd[m,1]>1){ # Need at least 2 samples to do the linearization
      ...
      data.cor[[m]]$dp17O.SMOWSLAP.CO2norm[test.carb] <-
        data.cor[[m]]$D17O.SMOWSLAP.per.meg.carbNorm[test.carb]/10^6 + 0.528*data.cor[[m]]$dp18O.SMOWSLAP.CO2norm[test.carb]
      # And note the type of correction
  ...
  #   ############################################################################################
  #   # Secondary data normalization for d18O and d17O
  #   ############################################################################################   
  #   # Check if the reactor has analyses of multiple carbonates (any Type.2 carbonate) If it does not, no normalization can be applied.
  #   flag.mult.carbstd <- matrix(-9999,length(data.cor),1) # default flag is -9999 = do not perform secondary correction
  #   for (m in 1:length(data.cor)){
  #     curCarbO2 <- subset(data.cor[[m]],data.cor[[m]]$Type.1 == "CarbonateStd" | data.cor[[m]]$Type.1 == "Carbonate")
  #     curCarbO2 <- unique(curCarbO2$Type.2) # Only keep the relevant column
  #     if (length(curCarbO2)>1){ # there are >1 carbonate standards
  #       flag.mult.carbstd[m,1] <- length(curCarbO2) # number of different carbonate Type.2's analyzed. Number is just for user - anything above -9999 will run
  #     }
  #   }
  #   
  #   
  # can only do the correction if there are 2+ analyzed carbonates THAT MATCH THE INPUT LIST
  #   
  #   # Make and apply the secondary data normalization.
  #   # Holding lists
  #   norm.2nd.model <-
  #     vector(mode = "list", length = length(data.cor)) # holding list for correlations
  #   norm.2nd.model.data <-
  ...
  #       
  #       # Apply secondary correction (line of best fit) to data
  #       # # Set up column for secondary-corrected d18O (default is no correction applied)
  #       # data.cor[[m]]$dp17O.SMOWSLAP.CO2norm <- data.cor[[m]]$dp17O.SMOWSLAP
  #       # data.cor[[m]]$dp18O.SMOWSLAP.CO2norm <- data.cor[[m]]$dp18O.SMOWSLAP
  #       # Index for the carbonate data (model only applies to carbonates, which have a blank correction)
  #       test.carb <-data.cor[[m]]$Type.1 == "CarbonateStd" |
  ...
  #       
  #       # Apply secondary correction to d17O data
  #       # Note that this requires knowledge of lambda-blank. We assume no change in D17O.
  #       lambda.blank = 0.528
  #       data.cor[[m]]$dp17O.SMOWSLAP.CO2norm[test.carb] <-
  #         data.cor[[m]]$dp17O.SMOWSLAP[test.carb] + 0.528 * (data.cor[[m]]$dp18O.SMOWSLAP.CO2norm[test.carb] - data.cor[[m]]$dp18O.SMOWSLAP[test.carb])
  #     } else {
  #       # No secondary correction is possible for this dataset (note that it may
  #       # still be required - not all traditional d18O values have been compiled
  #       # for comparison; TEHuth 4/23/2020)
  #       # Set up column for secondary-corrected d18O (default is no correction applied)
  #       data.cor[[m]]$dp17O.SMOWSLAP.CO2norm <- data.cor[[m]]$dp17O.SMOWSLAP
  #       data.cor[[m]]$dp18O.SMOWSLAP.CO2norm <- data.cor[[m]]$dp18O.SMOWSLAP
  #     }
  ...
  for (k in 1:(ncol(summary.SMOW) - 1)) {
    ...
    if (k == 1) {
      ...
      names(smow.all) = c("Date.Time", "d33", "d34", "D17O.SMOWSLAP.per.meg.carbNorm", "CorType")
      count <-
      ...
      names(smow.all.add) = c("Date.Time", "d33", "d34", "D17O.SMOWSLAP.per.meg.carbNorm", "CorType")
  ...
  for (rr in 1:2) {
    ...
    if (rr == 1) {
      ...
      curSMOW <- select(smow.all, Date.Time, d33, CorType)
    } else {
      # d34 is current variable
      curSMOW <- select(smow.all, Date.Time, d34, CorType)
    }
  ...
  plot.SMOW.allCor <-
    ggplot() + geom_point(
      ...
      aes(
        ...
        y = smow.all$D17O.SMOWSLAP.per.meg.carbNorm,
  ...
  # This is no longer used. The user must select which correction (basic, 
  # linear, segmented-basic, or segmented-linear) for each reactor and segment
  # # Automatically selected final data output. Note: this is auto-selected and selects the segmented correction first. If this is not available, the linear correction is used (given the rsq threshold = 0). The basic correction is selected last. While this is order of priority is generally useful (and differences between schemes are often small), there may be cases you wish to choose another correction scheme or one not coded here. To force the user to decide, the data sorting program only uses user-selected values - i.e., you must input what correction scheme to use.
  # if (segment.IPL[1, 1] == -9999) {
  #   # There are no segments, output the linear correction
  ...
  #                                             "R",
  #                                             prefix,
  #                                             "_dataAutoFinal_lineStats.csv",
  #                                             sep = "")
  #   write.csv(D17O.final, curFile.data.cor.final, row.names = FALSE)
  #   write.csv(D17O.final.line.stats,
  #             curFile.data.cor.final.line.stats,
  #             row.names = FALSE)
  # } else {
  #   # There are segments, output the segmented correction
  ...
  #                                               ".csv",
  #                                               sep = "")
  #     write.csv(D17O.final[[k]], curFile.data.cor.final, row.names = FALSE)
  #     write.csv(D17O.final.line.stats[[k]],
  #               curFile.data.cor.final.line.stats,
  #               row.names = FALSE)
  #   }
  # }
