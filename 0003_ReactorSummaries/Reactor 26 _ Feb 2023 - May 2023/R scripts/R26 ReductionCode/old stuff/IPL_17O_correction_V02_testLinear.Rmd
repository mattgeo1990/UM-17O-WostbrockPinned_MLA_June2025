#  test file to see if I can get the linear correction working when it is applied to smow and slap individuall
...
for (reactor.file.number in 16) {
  # for(reactor.file.number in 1:length(reactor.file.names)) {
  ...
  rm(list = ls()[!ls() %in% c("reactor.file.names", "reactor.file.number","path.data.red","tert.cor.option")])
  ...
  source(paste(path.to.functions, "correct.SMOW.SLAP.linearSMOW_V02.R", sep = ""))
  ...
  D17O.data.linearSMOW.cor.HOLD = correct.SMOW.SLAP.linearSMOW_V02(D17O.data)
  ...
  if (segment.IPL[1, 1] != -9999) {
    ...
    for (k in 1:nrow(segment.IPL)) {
      ...
      segment.cor.HOLD.linear <-
        correct.SMOW.SLAP.linearSMOW_V02(D17O.data.segment)
      if (k == 1) {
  ...
  for (m in 1:length(data.cor)) {
    # Values
    data.cor[[m]]$dp17O.final.2nd <- data.cor[[m]]$dp17O.final
    data.cor[[m]]$dp18O.final.2nd <- data.cor[[m]]$dp18O.final
    data.cor[[m]]$D17O.final.3rd <- data.cor[[m]]$D17O.per.meg
    
    # Type of correction applied
    data.cor[[m]]$dp17O.final.2nd.type <- 'no correction'
    data.cor[[m]]$dp18O.final.2nd.type <- 'no correction'
    data.cor[[m]]$D17O.final.3rd.type <- 'no correction'
  }
  ...
  for (m in 1:length(data.cor)){
    ...
    tert.meas.D17O[1] <- mean(subset(curData,Type.2 == tert.names.D17O[1])$D17O.per.meg) # NBS-18
    tert.meas.D17O[2] <- mean(subset(curData,Type.2 == tert.names.D17O[2])$D17O.per.meg) # NBS-19
    tert.meas.D17O[3] <- mean(subset(curData,Type.2 == tert.names.D17O[3])$D17O.per.meg) # IAEA-603
    tert.meas.D17O[4] <- mean(subset(curData,Type.2 == tert.names.D17O[4])$D17O.per.meg) # 102-GC-AZ01
    ...
    if (tert.cor.option==1){ # use 'best' available carbonate standard(s)
      ...
      if (sum(flag.tert[1:(len.flag.tert-1),1])>1){ # do 2-pt on NBS-18, NBS-19, and/or IAEA-603
        ...
        data.cor[[m]]$D17O.final.3rd[test.carb] <-
          data.cor[[m]]$D17O.per.meg[test.carb]*lm.norm.tert.slope + lm.norm.tert.intercept
        # And note the type of correction
        data.cor[[m]]$D17O.final.3rd.type[test.carb] <- paste("2-pt normalization applied to D17O for data.cor[[", m , "]] using ", cur.tert.names.D17O.2, ". slope=", round(lm.norm.tert.slope,2), " int=", round(lm.norm.tert.intercept,2), sep="")
        ...
        # Apply 2-pt tertiary correction to D17O data
        data.cor[[m]]$D17O.final.3rd[test.carb] <-
          data.cor[[m]]$D17O.per.meg[test.carb] + offset.tert
        # And note the type of correction
        data.cor[[m]]$D17O.final.3rd.type[test.carb] <- paste("1-pt normalization applied to D17O for data.cor[[", m , "]] using ", cur.tert.names.D17O, ". Offset=", round(offset.tert,2), " per meg", sep="")
        ...
        # data.cor[[m]]$D17O.final.3rd <- NA # carbonate data not useful in context of this code
        # test.water <-data.cor[[m]]$Type.1 == "WaterStd" |
        #   data.cor[[m]]$Type.1 == "Water"
        # data.cor[[m]]$D17O.final.3rd[test.water] <-
        #   data.cor[[m]]$D17O.per.meg[test.water] # Water data is still useful
        norm.tert.model[[m]] <- c("none")
        norm.tert.model.data[[m]] <- curData
        # And note the type of correction
        data.cor[[m]]$D17O.final.3rd.type[test.carb] <- paste("No tertiary normalization possible for data.cor[[", m , "]]", sep="")
      ...
      if (flag.tert[4]==1){ # do 1-pt (offset) on 102-GC-AZ01 (no international standards available)
        ...
        # Apply no correction to data - just copy the SMOW-SLAP-normalized data
        # Set up column for secondary-corrected d18O (default is no correction applied). No need this is done above
        # data.cor[[m]]$D17O.final.3rd[test.carb] <- data.cor[[m]]$D17O.per.meg[test.carb] # this is just the SMOW-SLAP corrected column copied over
        # test.water <-data.cor[[m]]$Type.1 == "WaterStd" |
        #   data.cor[[m]]$Type.1 == "Water"
        # data.cor[[m]]$D17O.final.3rd[test.water] <-
        #   data.cor[[m]]$D17O.per.meg[test.water] # Water data is still useful
        ...
        norm.tert.model.data[[m]] <- curData
        # And note the type of correction
        data.cor[[m]]$D17O.final.3rd.type[test.carb] <- paste("No tertiary normalization possible for this analysis, kept SMOW-SLAP corrected data", sep="")
        print(paste("No tertiary normalization possible for data.cor[[", m , "]]", sep=""))
      }
    }
  }
  ...
  for (m in 1:length(data.cor)) {
    if (flag.mult.carbstd[m,1]>1){ # Need at least 2 samples to do the linearization
      ...
      lm.norm.2nd <-
        lm(curData$dp18O.final ~ curData$trad.dp18O.co2.cc)
      lm.norm.2nd.slope <-
      ...
      data.cor[[m]]$dp18O.final.2nd[test.carb] <-
        (data.cor[[m]]$dp18O.final[test.carb] - lm.norm.2nd.intercept) / lm.norm.2nd.slope
      # And note the type of correction
      data.cor[[m]]$dp18O.final.2nd.type[test.carb] <- paste("secondary correction applied", sep="")
      ...
      data.cor[[m]]$dp17O.final.2nd[test.carb] <-
        data.cor[[m]]$D17O.final.3rd[test.carb]/10^6 + 0.528*data.cor[[m]]$dp18O.final.2nd[test.carb]
      # And note the type of correction
      data.cor[[m]]$dp17O.final.2nd.type[test.carb] <- paste("secondary correction applied", sep="")
      ...
      data.cor[[m]]$dp18O.final.2nd.type[test.carb] <- paste("no secondary correction", sep="")
      ...
      data.cor[[m]]$dp17O.final.2nd[test.carb] <-
        data.cor[[m]]$D17O.final.3rd[test.carb]/10^6 + 0.528*data.cor[[m]]$dp18O.final.2nd[test.carb]
      # And note the type of correction
      data.cor[[m]]$dp17O.final.2nd.type[test.carb] <- paste("secondary correction applied but only meaningful if D'17O correction was applied", sep="")
    }
  }
  ...
  #       lambda.blank = 0.528
  #       data.cor[[m]]$dp17O.final.2nd[test.carb] <-
  #         data.cor[[m]]$dp17O.final[test.carb] + 0.528 * (data.cor[[m]]$dp18O.final.2nd[test.carb] - data.cor[[m]]$dp18O.final[test.carb])
  #     } else {
  ...
  for (k in 1:(ncol(summary.SMOW) - 1)) {
    ...
    if (curData.line.flag == 1) {
      ...
      smow.SD = round(sd(as.numeric(cur.smow$D17O.final.3rd)), 0)
      smow.n = nrow(cur.smow)
      smow.range = round(max(as.numeric(cur.smow$D17O.final.3rd)) - min(as.numeric(cur.smow$D17O.final.3rd)))
    } else {
    ...
    if (k == 1) {
      ...
      smow.all <-
        data.frame(
          ...
          cur.smow$d33,
          cur.smow$d34,
          cur.smow$D17O.final.3rd,
          corType
        )
      # change column names (can't be done at same time apparently...)
      names(smow.all) = c("Date.Time", "d33", "d34", "D17O.final.3rd", "CorType")
      count <-
      ...
      names(smow.all.add) = c("Date.Time", "d33", "d34", "D17O.final.3rd", "CorType")
  ...
  for (r in 1:length(data.cor))
  {
    for (rr in 1:length(standards))
    {
      ...
      cur.std$D17O.accept <-
        cur.std$D17O.final.3rd * 0 + cur.accepted$D17O.per.meg.accepted
      cur.std$residual <-
        cur.std$D17O.final.3rd - cur.accepted$D17O.per.meg.accepted
      # calculate average info for the summary-standards list
      avg = round(mean(cur.std$D17O.final.3rd), 0)
      avg.resid = round(avg - cur.accepted$D17O.per.meg.accepted, 0)
      n = nrow(cur.std) # number of replicates
      SD = round(sd(cur.std$D17O.final.3rd), 0) # standard deviation of replicates
      if (rr == 1) {
